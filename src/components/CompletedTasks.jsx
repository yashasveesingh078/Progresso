...
const CompletedTasks = ({tasks,setTasks,draggedTask,setDraggedTask,darkMode}) => {
  ...
  const handleDrop = () => {
    const today = new Date()
      ...
      .split("T")[0];
    setTasks(
      tasks.map((task) =>task.id === draggedTask ? {...task,status: "completed",completedDate: today} : task)
    )
  }
  return (
    <div onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} className={`${darkMode ? "bg-gray-800" : "bg-gray-200"} rounded-2xl p-6 min-h-[500px]`}>
      <div className="flex items-center justify-between mb-6">
        <h1 className={`${darkMode ? "text-white" : "text-black"} text-3xl font-bold`}>Completed Tasks</h1>
        <span className="bg-pink-700 text-white px-4 py-2 rounded-full text-sm font-semibold">{completedTasks.length}</span>
      </div>
